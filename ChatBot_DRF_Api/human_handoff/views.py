...
def agent_send_message_view(request):
    ...
    try:
        ...
        # Check if session is escalated and assigned to this agent or unassigned
        try:
            handoff_session = HumanHandoffSession.objects.get(
                chat_session=chat_session,
                resolved_at__isnull=True
            )
            # If session is unassigned, assign it to this agent
...
def agent_upload_file_view(request):
    """
    Upload file as agent for chat message
