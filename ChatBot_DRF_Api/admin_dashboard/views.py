...
def check_agent_limit_view(request):
    ...
    try:
        ...
        response_data = {
            ...
            'plan_name': active_assignment.plan.plan_name_display,
        }
...
def update_user_plan(upgrade_request):
    ...
    try:
        ...
        # Get or create the plan for this company
        plan, created = Plan.objects.get_or_create(
            plan_name=plan_name_db,
            company_name=upgrade_request.company_id,
            defaults={
                'max_agents': Plan.get_predefined_plans().get(plan_name_db, {}).get('max_agents', 2),
                'price': Plan.get_predefined_plans().get(plan_name_db, {}).get('price', 0),
                'is_active': True
            }
        )
