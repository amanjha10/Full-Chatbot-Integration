...
export default function RealTimeChat({
  ...
  const fetchChatHistory = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/human-handoff/agent/sessions/${sessionId}/messages/`,
        {
          headers: {
  ...
  useEffect(() => {
    const wsUrl = `${import.meta.env.VITE_WS_BASE_URL || "wss://bot.spell.com.np"}/ws/chat/${companyId}/${sessionId}/`;
      ...
      if (userType === "agent") {
        ...
        if (inputMessage.trim()) {
          response = await fetch(
            `${apiBaseUrl}/human-handoff/agent/send-message/`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        if (fileList.length > 0) {
          ...
          const uploadEndpoint = `${apiBaseUrl}/chat/upload/`;
