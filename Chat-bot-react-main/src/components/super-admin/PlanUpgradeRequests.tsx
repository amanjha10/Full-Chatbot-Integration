...
import { getPlanUpgradeRequests } from "../../api/get";
import { reviewPlanUpgradeRequest as submitReview } from "../../api/post";
...
const PlanUpgradeRequests: React.FC = () => {
  ...
  const {
    ...
    getPlanUpgradeRequests().then((res) => res.data)
  );

  // Extract requests from response - handle both array and object with results
  ...
  const submitReviewRequest = async () => {
    ...
    setLoading(true);
    try {
      await submitReview(selectedRequest.id, reviewAction, "");
      ...
      message.error("Failed to process review. Please try again.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex justify-between items-center">
            ...
            <Tag color="blue">
              {Array.isArray(requests)
                ? requests.filter(
                    (r: PlanUpgradeRequest) => r.status === "PENDING"
                  ).length
                : 0}{" "}
              Pending
            </Tag>
