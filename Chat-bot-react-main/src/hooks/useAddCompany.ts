...
export const useAddCompany=({setLoading,handleCloseModal,mutate,companyId}:AddCompanyProps)=>{
  const {data}=useSWR(companyId?`/auth/list-admins/?admin_id=${companyId}`:'')
  ...
  const onSubmit=(data:FormType)=>{
    setLoading(true)
    if(companyId){
     editCompany(data,companyId).then(()=>{
